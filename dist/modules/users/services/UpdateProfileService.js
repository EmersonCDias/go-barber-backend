...
let UpdateProfileService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async run({
    ...
    if (!user) throw new _AppErrors.default('User not found');
    const userEmailExists = await this.usersRepository.findUserByEmail(email);

    if (userEmailExists && userEmailExists.id !== user_id) {
      throw new _AppErrors.default('E-mail already in use');
    }

    user.name = name;
    user.email = email;
    if (password && !old_password) throw new _AppErrors.default('Old password not informed');

    if (password && old_password) {
      const checkOldPassword = await this.hashProvider.compareHash(old_password, user.password);
      if (!checkOldPassword) throw new _AppErrors.default('Old password does not match');
