...
let SendForgotPasswordEmailService = (_dec = (0, _tsyringe.injectable)(), _dec2 = function (target, key) {
  ...
  async run({
    ...
    if (!userToken) throw new _AppErrors.default('Token is missing');
    const user = await this.usersRepository.findUserById(userToken.user_id);
    if (!user) throw new _AppErrors.default('User does not exists');
    const tokenCreatedAt = userToken.created_at;

    if ((0, _dateFns.differenceInHours)(Date.now(), tokenCreatedAt) > 2) {
      throw new _AppErrors.default('Token expired');
    }
